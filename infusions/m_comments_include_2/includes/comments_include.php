...
function showcomments($ctype, $cdb, $ccol, $cid, $clink) {
	...
	if (dbrows($result)) {
		...
		while ($data = dbarray($result)) {
			echo "<tr><td colspan='2' width='100%' class='tbl2'>\n";
			...
			echo "<a href='".FUSION_REQUEST."#c".$data['comment_id']."' id='c".$data['comment_id']."' name='c".$data['comment_id']."'>#".$i."</a></td></tr>
			<tr><td align='left' width='17%' class='tbl' rowspan='2'>";
			if ($data['user_name']) {
					$data1=dbarray(dbquery("SELECT * FROM ".DB_USERS." WHERE user_name='".$data['user_name']."'"));
					if ($data1['user_avatar'] && file_exists(IMAGES."avatars/".$data1['user_avatar'])) {
						echo "<a href='".BASEDIR."profile.php?lookup=".$data['comment_name']."'><img src='".IMAGES."avatars/".$data1['user_avatar']."' alt='аватар' border='0'/></a><br />\n";
					}
					else{
					echo "<a href='".BASEDIR."profile.php?lookup=".$data['comment_name']."'><img src='".IMAGES."avatars/noavatar.gif' alt='аватар' border='0'/></a><br />\n";
					}
					
					echo "<span class='comment-name'><a href='".BASEDIR."profile.php?lookup=".$data['comment_name']."'>".$data['user_name']."</a></span><br>\n";
					echo "<span class='small'>";
					if ($data1['user_level']==0) {echo "<b>Гость</b>";
					}else{
						if ($settings['forum_ranks']) {
							echo show_forum_rank($data1['user_posts'], $data1['user_level']);
						} else {
							echo getuserlevel($data1['user_level']);
						}
					}
					echo "</span>\n";
			} else {
				echo "<img src='".IMAGES."avatars/noavatar.gif' alt='нет аватара' border='0'/><br />\n";
				echo "<span class='comment-name'>".$data['comment_name']."</span>\n";
			}
			
			echo "</td><td width='85%' cellpadding='0' cellspacing='0' valign='top' border='1'>
			<table width='100%' valign='top' cellpadding='0' cellspacing='5'><tr><td align='right' valign='top'>
			".$locale['global_071'].showdate("longdate", $data['comment_datestamp'])."<hr width='80%' align='right' height='2'></td></tr>";
			echo "
			<tr><td align='left' class='tbl' cellpadding='10' valign='top' width='100%'><!--comment_message-->".nl2br(parseubb(parsesmileys($data['comment_message'])))."</td></tr>
			</table>
			</td></tr>
			<tr><td><hr width='80%' align='left' height='1'>";
			if ((isset($data1['user_sig']))&&($data1['user_sig']!='')){echo "<br>".nl2br(parseubb(parsesmileys($data1['user_sig'])));}
			echo "</td></tr>\n";
			$i++;
		}
		echo "</table>";
