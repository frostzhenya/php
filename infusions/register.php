...
if (isset($_GET['activate'])) {
	...
	$rq_query = dbquery("SELECT inf_id FROM ".DB_PREFIX."infusions WHERE inf_folder='register_quest'");
	if (dbrows($rq_query) != 0){
		...
		$answer =stripinput(trim($_POST['answer']));
		$rq_query1 =dbarray(dbquery ("SELECT * FROM ".DB_PREFIX."register_quest WHERE rq_hash='$quest'"));
		if ($rq_query1['rq_answer'] != $answer){
			$error .= $locale['rq_err_register']."<br />\n";
		}
	}
	...
	if (dbrows($rq_query) != 0){
		...
		$rq_query = dbarray(dbquery ("SELECT rq_question FROM ".DB_PREFIX."register_quest ORDER BY RAND() LIMIT 0,1"));
		$rq_quest = $rq_query['rq_question'];
			echo "<tr>\n";
			echo "<td class='tbl'>".$locale['rq_please']."<span style='color:#ff0000'>*</span></td>\n";
			echo "<td class='tbl'>$rq_quest<br /><input type='hidden' value='".md5($rq_quest)."' name='quest'><input type='text' name='answer' maxlength='500' class='textbox' style='width:400px;' /></td>\n";
			echo "</tr>\n";
	}
	if ($settings['email_verification'] == "0") {
