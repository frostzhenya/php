		...
		if (dbrows($move_result)) {
			...
			if (!isset($_POST['new_forum_id']) && !$f_post_blo) {
				...
				if ($move_posts_add) {
					...
					if ($posts_ex) {	
						$result = dbquery("UPDATE ".DB_POSTS." SET forum_id='".$_POST['new_forum_id']."', thread_id='".$_POST['new_thread_id']."', post_datestamp='".time()."' WHERE post_id IN (".$move_posts_add.")");
						...
						if (!dbcount("(post_id)", DB_POSTS, "thread_id='".$pdata['thread_id']."'")) {
							$result = dbquery("UPDATE ".DB_FORUMS." SET forum_lastpost='".$old_thread['post_datestamp']."', forum_postcount=forum_postcount-".$num_posts.", forum_threadcount=forum_threadcount-1, forum_lastuser='".$old_thread['post_author']."' WHERE forum_id='".$pdata['forum_id']."'");
